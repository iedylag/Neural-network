...
df_train <- subset(df_train, select = -Name)
df_train <- subset(df_train, select = -Cabin)
df_train <- subset(df_train, select = -Ticket)
...
df_new <- as.matrix(df_new_data) # Convert to numeric matrix
predictions <- predict(nn, newdata = df_new)

# Wyświetlenie wyników predykcji dla każdego pasażera
for (i in 1:nrow(predictions)) {
  if (predictions[i] == 0) {
    print(paste("Pasażer", i, ": Nie przeżył"))
  } else {
    print(paste("Pasażer", i, ": Przeżył"))
  }
}

# Predykcja dla nowych danych
new_data <- data.frame(Pclass = 3, Sex = "female", Age = 24, SibSp = 1,
                       Parch = 2, Fare = 7.5, Embarked="Q")
